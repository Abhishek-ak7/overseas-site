...
import { Search, MessageCircle, User, Phone, Mail, MapPin, Calendar, Filter, GraduationCap, BookOpen } from "lucide-react"
...
const queryTypeIcons: Record<string, any> = {
  university_selection: GraduationCap,
  visa_guidance: MapPin,
  document_help: BookOpen,
...
export default function ConsultationsPage() {
  ...
  useEffect(() => {
    fetchInquiries()
  }, [pagination.page, statusFilter, priorityFilter, queryTypeFilter])
  ...
  const fetchInquiries = async () => {
    try {
      ...
      if (queryTypeFilter && queryTypeFilter !== 'all') params.append('query_type', queryTypeFilter)
      if (searchTerm) params.append('search', searchTerm)

      const response = await fetch(`/api/admin/consultation-inquiries?${params}`)
      const data = await response.json()

      console.log('API Response:', data)
      if (data.success) {
        console.log('Inquiries received:', data.inquiries)
        setInquiries(data.inquiries)
        setPagination(data.pagination)
      } else {
        throw new Error(data.message || 'Failed to fetch consultation inquiries')
      }
    } catch (error) {
      console.error('Error fetching inquiries:', error)
      toast({
        title: "Error",
        description: "Failed to load consultation inquiries",
        variant: "destructive"
      })
    } finally {
      setLoading(false)
  ...
  const updateInquiryStatus = async (inquiryId: string, updates: Record<string, any>) => {
    setIsUpdating(true)
    try {
      const response = await fetch(`/api/admin/consultation-inquiries/${inquiryId}`, {
        method: 'PATCH',
        headers: {
      ...
      if (result.success) {
        ...
        toast({
          title: "Success",
          description: "Consultation inquiry updated successfully"
        })
      } else {
      ...
      toast({
        title: "Error",
        description: "Failed to update consultation inquiry",
        variant: "destructive"
      })
    } finally {
      setIsUpdating(false)
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Consultation Inquiries</h1>
          <p className="text-gray-600 mt-1">Manage consultation requests from students</p>
        </div>
      </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              ...
              <Select value={queryTypeFilter} onValueChange={setQueryTypeFilter}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="document_help">Document Help</SelectItem>
      ...
      <div className="grid gap-4">
        {inquiries.map((inquiry) => {
          const QueryIcon = queryTypeIcons[inquiry.queryType] || MessageCircle
          return (
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex gap-2">
                        ...
                        <Badge variant="outline" className="flex items-center gap-1">
                          ...
                          {queryTypeLabels[inquiry.queryType]}
                        </Badge>
                      </div>
                    </div>
